...
def project(
    ...
    phi = torch.tensor(phi, dtype=torch.float32, device=device,
                         requires_grad=True)
    theta_optimizer = torch.optim.Adam([theta] + list(noise_bufs.values()), betas=(0.9, 0.999),
                                 lr=hyperparameters.cam_latent_lr)
    phi_optimizer = torch.optim.Adam([phi] + list(noise_bufs.values()), betas=(0.9, 0.999),
                                 lr=hyperparameters.cam_latent_lr)
    #################################
